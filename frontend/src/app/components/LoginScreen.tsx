...
export function LoginScreen({ allowBack = true, onBack, onLogin }: LoginScreenProps) {
  ...
  const handleAction = () => {
    ...
    if (mode === "login") {
      ...
      if (validateLogin()) {
        const users = getUsers();
        const fullName = users[email].name;
        const role = users[email].role || "client";
        const phone = users[email].phone || "";
        onLogin(fullName, false, role, email, phone);
      }
    } else {
      if (signupStep === 1) {
          ...
          {mode === "login" && (
            <>
              ...
              <div className="flex items-center gap-3 mb-10">
                <div className="w-24 h-16 rounded-2xl bg-[#12121a] border border-[#c9a84c]/30 shadow-[0_0_28px_rgba(201,168,76,0.14)] overflow-hidden flex items-center justify-center">
                ...
                <div>
                  <div className="flex items-baseline gap-1">
                    <span style={{ fontFamily: "'Outfit', sans-serif", fontWeight: 800, fontSize: 23, color: "#f0ede8", letterSpacing: "0.06em" }}>
                      RENT
                    </span>
                    <span style={{ fontFamily: "'Outfit', sans-serif", fontWeight: 800, fontSize: 23, color: "#c9a84c", letterSpacing: "0.06em" }}>
                      EC
                    </span>
                  </div>
                  <div className="flex items-center gap-2 mt-0.5">
                    <span className="h-px w-5 bg-[#c9a84c]" />
                    <span style={{ fontFamily: "'Outfit', sans-serif", fontSize: 9, color: "#7a7890", letterSpacing: "0.22em", fontWeight: 700 }}>
